...
- **The LLM feature grew a discovery mode, still grounded, still not a chatbot.** Beyond the per-recipe assistant, a Smart Recipe Finder on `/recipes` takes a natural-language query ("quick vegan dinners with chickpeas") and returns real recipes from the existing catalog — one query, one answer, no back-and-forth conversation. The model can only select from real recipe IDs; anything it invents is discarded server-side before the response is built, so the frontend never renders a model-supplied title or nutrition figure, only what's actually in `data.json`. Full account, including how a Codex-drafted plan for this got independently reviewed and corrected before implementation, in PLAN.md §3.33.
...
*Beyond the suggested list:* a persistent dietary/interest profile that auto-applies as your default filter state on a fresh visit; a difficulty filter; a proactive dietary-conflict banner on the recipe detail page; quick prompt chips on the recipe assistant ("Suggest substitutions," "Make it vegan," etc.); a Smart Recipe Finder on `/recipes` for natural-language discovery across the existing catalog, with its own discovery chips and a visible note whenever your saved dietary profile is being applied to results.
...
- **The Smart Recipe Finder's saved-dietary-profile handling is a soft instruction to the model, not a hard filter.** Recipe *IDs* are hard-enforced (anything the model returns that isn't a real recipe in the catalog is discarded server-side, always) — but "respect the user's saved dietary restrictions" is prompt guidance, not a server-side filter on results. Observed live: a vegan-profile query returned 5 genuinely vegan matches and 1 that was only vegetarian. Documented rather than silently accepted; a stricter version would filter/re-rank matches server-side against each recipe's real `dietary[]` array.
