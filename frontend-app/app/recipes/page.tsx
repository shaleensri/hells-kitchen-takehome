  ...
  return (
    <div className={`container ${styles.page}`}>
      ...
      <form action="/recipes" method="GET" className={styles.searchForm}>
        ...
        />
        {tags?.map((t) => <input key={`tags-${t}`} type="hidden" name="tags" value={t} />)}
        {ingredients?.map((i) => <input key={`ing-${i}`} type="hidden" name="ingredients" value={i} />)}
        {dietary?.map((d) => <input key={`diet-${d}`} type="hidden" name="dietary" value={d} />)}
