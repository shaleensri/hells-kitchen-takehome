...
**168/168 tests passing** (`npm test` from root or any workspace, verified against a fresh run — not computed by hand, since stale/miscounted test totals have bitten this file before, §3.28): 85 backend + 10 shared + 73 frontend (167 from §3.29 + 1 new `llm.test.ts` case from §3.30's history-role fix). The search form's own hidden-input rendering (the bug in §3.22), the stretched-link card restructure, and every Iteration 5/6 and §3.29 UI flow aren't Vitest-covered where they're pure wiring — verified via Playwright instead (§3.17, §3.19, §3.20, §3.25, §3.26, §3.29); the LLM proxy is additionally verified with genuine live calls to the real OpenAI API (§3.26, §3.29), not just the mocked unit-test suite. `npm run dev` in both `backend-app` and `frontend-app` now serves a real, working website at `localhost:3000/recipes`, and the real deploy is live (§3.27). Full detail: §3.14 (Iteration 1's architecture snag), §3.15-§3.16 (backend review passes), §3.17 (Iteration 3's build), §3.20 (the spec-sheet redesign), §3.22 (this review's fixes), §3.25 (Iteration 5), §3.26 (Iteration 6), §3.27 (deploy), §3.28 (trust-proxy fix), §3.29 (LLM panel fixes), §3.30 (history-role security fix).
...
### Iteration 10 — Expanded dataset (stretch, added at user request, §3.21) — 🔲
- Decide between the two paths in §3.21 (LLM-generated synthetic recipes, leaning towards this, vs. importing from a real public recipe API) before starting — genuinely undecided, not a default.
- Generate/import happens **offline via a script**, not a live runtime fetch — output is a bigger `data.json` (or a follow-on dataset file), loaded the same way at boot as today (§5.4). No new request-path dependency, no new failure mode for the deployed app.
- Any new ingredient introduced must get real dietary/allergen/nutrition values **and** unit-conversion entries (§3.9) in the same pass — otherwise new recipes show up `partial`/`unresolved` and undercut the entire point of this iteration (showing the app at its best with more data).
- Run the seed-validation script (§3.4) against the expanded dataset before trusting it — it exists for exactly this case.
