...
**167/167 tests passing** (`npm test` from root or any workspace, verified against a fresh run — not computed by hand, since stale/miscounted test totals have bitten this file before, §3.28): 84 backend + 10 shared + 73 frontend (144 from §3.28 + 4 backend `llm.test.ts`/`app.test.ts` additions from §3.29's history support + 19 frontend `markdownLite.test.ts`/`askHistory.test.ts`/`api.test.ts` additions from §3.29). The search form's own hidden-input rendering (the bug in §3.22), the stretched-link card restructure, and every Iteration 5/6/9 UI flow aren't Vitest-covered where they're pure wiring — verified via Playwright instead (§3.17, §3.19, §3.20, §3.25, §3.26, §3.29); the LLM proxy is additionally verified with genuine live calls to the real OpenAI API (§3.26, §3.29), not just the mocked unit-test suite. `npm run dev` in both `backend-app` and `frontend-app` now serves a real, working website at `localhost:3000/recipes`, and the real deploy is live (§3.27). Full detail: §3.14 (Iteration 1's architecture snag), §3.15-§3.16 (backend review passes), §3.17 (Iteration 3's build), §3.20 (the spec-sheet redesign), §3.22 (this review's fixes), §3.25 (Iteration 5), §3.26 (Iteration 6), §3.27 (deploy), §3.28 (trust-proxy fix), §3.29 (LLM panel fixes).
...
**New tests:** `markdownLite.test.ts` (14 — inline bold/code parsing, bullet vs. numbered list detection, mixed-content fallback to a plain paragraph, and a regression case built directly from real live model output) + `askHistory.test.ts` (4 — the pure append/cap-at-12 logic) + 2 new `llm.test.ts` cases (history included as alternating messages in the correct order; backward-compatible with no history) + 2 new `app.test.ts` cases (a well-formed history array reaches the provider correctly; malformed/oversized entries are dropped/truncated rather than 400ing) + 1 new `api.test.ts` case (history included in the POST body). **167 tests passing** (144 + 23 new), typecheck clean, clean production build. Backend redeployed to Railway with all three fixes live.
